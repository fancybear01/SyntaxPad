...
Небольшой редактор кода на Python (PyQt5) для быстрых правок и учебных примеров. Минимум лишнего, только удобные вещи: подсветка, автоскобки, автоотступы, запуск скрипта, поиск/замена и подсказки по встроенным функциям.
...
- Перенос строк по ширине редактора (View → Word Wrap)
- Поиск и замена по документу (View → Search/Replace или Ctrl+F) с опциями Match case и Whole word
- Сохранение настроек (тема, размер шрифта, перенос строк, calltips) в файле `~/.syntaxpad.json`
...
- Ctrl+0 – сбросить размер шрифта
 - Ctrl+F – открыть панель поиска/замены
